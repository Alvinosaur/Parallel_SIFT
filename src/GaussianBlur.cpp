...
void Gaussian_Blur::convolve_rows_mpi(Image & img, int* result, 
...
void Gaussian_Blur::convolve_cols_mpi(Image & img, int* result, 
    ...
    for(r = start; r < end; r++){
        for(c = 0; c < cols; c++){
            ...
            for(i = 0; i < K; i++){
                ...
                sum = sum + distrib[i]* (float)img.get(r, c1);
            }
            new_val = (int)sum;
            result[r*cols + c] = sum;
        }
